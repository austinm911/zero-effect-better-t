 ...
 * @see https://github.com/rocicorp/mono/blob/main/packages/zero-pg/src/postgres-connection.ts
 */
export class EffectDrizzleConnection<R = never>
	implements DBConnection<PgRemoteDatabase>
{
	...
	async transaction<TRet>(
		fn: (tx: DBTransaction<PgRemoteDatabase>) => Promise<TRet>,
	): Promise<TRet> {
		const transactionAdapter = new EffectDrizzleTransaction<R>(
			this.#drizzle,
			this.#sqlClient,
			this.#runtime,
		)
 ...
 * @category adapters
 */
class EffectDrizzleTransaction<R = never>
	implements DBTransaction<PgRemoteDatabase>
{
	/**
 ...
 * @category constructors
 */
export function zeroEffectDrizzleProcessor<TSchema extends Schema, R = never>(
	schema: TSchema,
	drizzle: PgRemoteDatabase,
	sqlClient: SqlClient.SqlClient,
	runtime: Runtime.Runtime<R>,
...
> {
	const zqlDatabase = zeroEffectDrizzle<TSchema, R>(
		schema,
		drizzle,
		sqlClient,
		runtime,
